 ...
 *  parameterized makeCheckimage(), added checker.h, added static keyword
 */
...
/* Rick Ramstetter: parameterized it */
/* void makeCheckImage(void) */
void makeCheckImage(GLubyte *_checkImage,
		    ...
		    int _checkImageHeight)
{
   ...
   for (i = 0; i < _checkImageHeight; i++) {
      for (j = 0; j < _checkImageWidth; j++) {
   ...
   */
   ...
   for (i = 0; i < _checkImageHeight; i++) {
       for (j = 0; j < _checkImageWidth; j++) {
	   c = ((((i&0x8)==0)^(((j&0x8))==0)))*255;
	   offset = (i * _checkImageWidth * 4) + (j * 4);
	   *(_checkImage + offset + 0) = (GLubyte) c;
	   *(_checkImage + offset + 1) = (GLubyte) c;
	   *(_checkImage + offset + 2) = (GLubyte) c;
	   *(_checkImage + offset + 3) = (GLubyte) 255;
       }
   }
}
